...
class GetChatsRequestMarshaller : io.grpc.MethodDescriptor.Marshaller<GetChatsRequestProto> {
    override fun stream(v: GetChatsRequestProto): java.io.InputStream {
        ...
        if (v.filter.isNotEmpty()) cos.writeString(5, v.filter); if (v.cursor.isNotEmpty()) cos.writeString(6, v.cursor)
        cos.flush(); return java.io.ByteArrayInputStream(baos.toByteArray())
    }
...
class GetChatsResponseMarshaller : io.grpc.MethodDescriptor.Marshaller<GetChatsResponseProto> {
    ...
    override fun parse(s: java.io.InputStream): GetChatsResponseProto {
        ...
        return GetChatsResponseProto(chats, nextCursor, hasMore)
    }
}
...
class SetSelfDestructTimerRequestMarshaller : io.grpc.MethodDescriptor.Marshaller<SetSelfDestructTimerRequestProto> {
    override fun stream(v: SetSelfDestructTimerRequestProto): java.io.InputStream {
        val baos = java.io.ByteArrayOutputStream(); val cos = com.google.protobuf.CodedOutputStream.newInstance(baos)
        if (v.roomId.isNotEmpty()) cos.writeString(1, v.roomId); cos.writeInt32(2, v.timerSeconds)
        cos.flush(); return java.io.ByteArrayInputStream(baos.toByteArray())
    }
    override fun parse(s: java.io.InputStream): SetSelfDestructTimerRequestProto = SetSelfDestructTimerRequestProto()
}

class SetSelfDestructTimerResponseMarshaller : io.grpc.MethodDescriptor.Marshaller<SetSelfDestructTimerResponseProto> {
    override fun stream(v: SetSelfDestructTimerResponseProto): java.io.InputStream = java.io.ByteArrayInputStream(byteArrayOf())
    override fun parse(s: java.io.InputStream): SetSelfDestructTimerResponseProto {
        val cis = com.google.protobuf.CodedInputStream.newInstance(s); var ok = false; var err = ""
        while (!cis.isAtEnd) { val tag = cis.readTag(); if (tag == 0) break; when (com.google.protobuf.WireFormat.getTagFieldNumber(tag)) { 1 -> ok = cis.readBool(); 2 -> err = cis.readString(); else -> cis.skipField(tag) } }
        return SetSelfDestructTimerResponseProto(ok, err)
    }
}
