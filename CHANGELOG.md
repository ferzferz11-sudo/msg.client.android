...
- Сервер не отправлял push когда ChatV2 stream был подключён к комнате (пользователь считался "в комнате")
- `ChatListActivity.onResume()` — stream переключается на пустую комнату при возврате в список чатов
- `ScreenStateReceiver` — при выключении экрана stream переключается на "" → сервер отправляет push
