...
### Известные проблемы
- **Агент завершается сразу после запуска** — `hermes_remote_agent.py` падает при подключении к серверу (ошибка в скрипте, не в клиенте)
- **Токены не фильтруются по пользователю** — `listAgentTokens` возвращает все токены из БД
