...
**Критические ошибки чата (Critical):**
- **Отправка и прокрутка**: Исправлена кнопка отправки и внедрен авто-скролл вниз при отправке сообщения. Теперь новые сообщения не скрываются под полем ввода.
- **Интеллектуальный скролл**: Список сообщений теперь автоматически «подпрыгивает» вместе с клавиатурой, сохраняя фокус на последних сообщениях.
- **Цитирование (Reply)**: Полностью восстановлено цитирование. Исправлена визуальная составляющая, Bold-стилизация имени автора и корректное отображение при ответе стикерами/фото.
- **Устойчивость данных**: Исправлена потеря цитат при перезагрузке истории или открытии через пуш-уведомления. Цитаты теперь корректно отображаются даже при ответе самому себе.

**Стабильность и локализация (Medium):**
- **Локализация**: Внедрена `BaseActivity`, что решило проблему самопроизвольного сброса языка на английский при перезапуске или открытии приложения через уведомления.
- **Статус пользователя**: Устранено мерцание статуса «не в сети» при входе в чат. Теперь информативное время последнего захода отображается мгновенно или скрывается до загрузки, не вводя в заблуждение.
- **Исправление времени**: Все временные метки возвращены к миллисекундам для идеальной совместимости с базой данных и сервером.
- **Оптимизация списка**: Дополнительно ускорена прокрутка списка чатов за счет более агрессивного кеширования аватаров и плоской иерархии `item_chat.xml`.
...
**Управление фоновым соединением (Feature):**
- В раздел «Безопасность» добавлена настройка «Фоновое соединение».
- Позволяет включать/выключать Foreground Service для работы в фоне.
- Отключено по умолчанию для экономии заряда (требует ручного включения для мгновенных уведомлений).

### Оптимизировано

**Производительность чат-листа (UI/UX):**
- Убрано мерцание и изменение цвета тулбара при прокрутке списка (отключен liftOnScroll).
- Оптимизирован RecyclerView: увеличен пул переиспользуемых ViewHolders для чатов (до 30).
- Уменьшено количество тяжелых вызовов `setTypeface`, `setBackgroundResource` и обновлений текста в адаптере чатов (обновляются только при изменении данных).
- Глобальная фильтрация и сортировка списка чатов вынесена в фоновый поток (Dispatchers.Default), что освобождает Main Thread.
- Исправлено мерцание фона AppBarLayout за счет удаления конфликтующих настроек прозрачности.

**Динамические табы:**
- Полностью переписан механизм обновления табов (AI, Компании, Архив).
- Таб «ИИ чаты» теперь скрывается, если нет активных ИИ-сессий, и корректно работает при переключении.
- Устранена проблема «некликабельности» табов из-за их постоянной пересоздачи.
- ИИ-чаты в архиве больше не вызывают отображение пустого таба «ИИ чаты».

### Исправлено

**Системное уведомление и локализация (Medium):**
- Заголовок уведомления теперь использует локализованное имя приложения («Лава» / «Lava») вместо «Lavender Messenger».
- Текст уведомления сокращен до «Подключено» / «Connected» (убрано лишнее «получение сообщений»).
- В текст уведомления добавлен счетчик новых сообщений: например, «Подключено (5 новых)».
- Исправлено отображение статуса «Вход по отпечатку» — теперь он корректно отключен по умолчанию.
