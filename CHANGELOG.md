...
- **Tab-based Settings** — новая настройка с вкладками:
  - «Шлюз» — SSH туннель для подключения через удалённый сервер. После подключения форма скрывается, показывается IP шлюза и статус
  - «Токен» — генерация JWT токена, управление агентом (start/stop), список активных токенов
- **Инструкции в настройках** — пошаговые инструкции для обоих режимов подключения
- **Статус подключения на тулбаре чата** — показывает тип подключения (шлюз IP / токен)
- **Управление агентом в чате** — кнопки Start/Stop в статус-баре
- **Сохранение выбора агента** — выбранный агент сохраняется между перезапусками приложения
- **Команды агента** — быстрые команды через кнопку в поле ввода (help, status, logs, deploy, restart, git, docker, ps, df, uptime)
- **Авто-прокрутка чата** — прокрутка к новым сообщениям
