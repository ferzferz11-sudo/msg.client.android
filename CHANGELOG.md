...
- 🐛 **Регистрация выбрасывала на рабочий стол**: `startActivity+finish` создавал гонку фокуса между двумя экземплярами ChatListActivity — система теряла фокус и приложение закрывалось. Заменено на `recreate()` как в логине.
- 🧹 **Очистка кэша при выходе**: убрана очистка кэша при входе/логине (лишняя операция, данные нового пользователя не пересекаются со старыми). Добавлена очистка в `logout()` и `deleteProfile()` — все данные пользователя удаляются с устройства при выходе из профиля.
- 🐛 **Logout показывал старые чаты**: при выходе из профиля список чатов оставался виден за шторкой авторизации. Теперь `logout()` синхронно очищает Room DB и пересоздаёт активити через `FLAG_ACTIVITY_NEW_TASK|CLEAR_TASK` — новый экземпляр показывает auth dialog на чистом экране.
